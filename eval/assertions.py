...
REQUIRED_PM_SECTIONS = ("总览", "行情与风险快照", "隔夜事件要点", "证据与不足")
...
def assert_required_sections(profile: str, report_md: str) -> list[str]:
    return [f"missing section: {s}" for s in REQUIRED_PM_SECTIONS if s not in report_md]
...
def run_hard_assertions(run_dir: Path, profile: str) -> list[str]:
    ...
    errors.extend(assert_required_sections(profile, report_md))
